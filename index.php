    ...
    <div class="main">
        ...
        <?php while ($row = mysqli_fetch_assoc($resultUtilisateurs)) : ?>
            <div class='profile-card'>
                <h2><?= htmlspecialchars($row['Pseudonyme'], ENT_QUOTES, 'UTF-8') ?></h2>
                <p>Nom: <?= htmlspecialchars($row['Nom'], ENT_QUOTES, 'UTF-8') ?></p>
                <p>Prénom: <?= htmlspecialchars($row['Prenom'], ENT_QUOTES, 'UTF-8') ?></p>
            ...
            <div id='additional-info-<?= $row['id'] ?>' class='additional-info hidden'>
                <h3><?= htmlspecialchars($row['Pseudonyme'], ENT_QUOTES, 'UTF-8') ?></h3>
                <p>Nom: <?= htmlspecialchars($row['Nom'], ENT_QUOTES, 'UTF-8') ?></p>
                <p>Prénom: <?= htmlspecialchars($row['Prenom'], ENT_QUOTES, 'UTF-8') ?></p>
                <p>Biographie: <?= htmlspecialchars($row['Biographie'], ENT_QUOTES, 'UTF-8') ?></p>
                <p>Pays: <?= htmlspecialchars($row['Pays'], ENT_QUOTES, 'UTF-8') ?></p>
                <p>Langue: <?= htmlspecialchars($row['Langue'], ENT_QUOTES, 'UTF-8') ?></p>
                <p>Genre de jeux: <?= htmlspecialchars($row['GenreJeux'], ENT_QUOTES, 'UTF-8') ?></p>
                <p>Style de gameplay: <?= htmlspecialchars($row['StyleGameplay'], ENT_QUOTES, 'UTF-8') ?></p>
                <p>Type de recherche: <?= htmlspecialchars($row['TypeRecherche'], ENT_QUOTES, 'UTF-8') ?></p>
